...
function getTimeOut(ts) {
  ...
  let surplus_time = parseInt(ts) - curr_time;
  if (surplus_time < 0) {
    return false;
  } else { //处理成时间字符串
    let time_d = parseInt(surplus_time / 86400);
    let time_h = zero_fill(parseInt((surplus_time % 86400) / 3600), 2);
    let time_m = zero_fill(parseInt((surplus_time % 3600) / 60), 2);
    let time_s = zero_fill(parseInt((surplus_time % 60)), 2);
    return time_d + '天' + time_h + ':' + time_m + ':' + time_s;
  }
